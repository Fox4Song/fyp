...
model = TransformerEncoder(
    x_dim, y_dim, r_dim=128, encoder_layers=8, encoder_heads=8
).to(device)
...
for it in range(TRAINING_ITERATIONS + 1):
    ...
    #    shapes: [B, L], [B, L], [B, L]
    input_batch, label_batch, mlm_mask_batch, attention_mask_batch = (
        polygon_reader.generate_masked_polygon_batch()
    )
    ...
    out = model(src, labels=label_batch, mlm_mask=mlm_mask_batch, attention_mask=attention_mask_batch)
...
final_checkpoint_path = f"final_tf_mlm_model_{total_params}_params.pt"
torch.save(
    {
