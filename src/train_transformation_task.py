...
def transform_train(model_name, iters, plot_after, device, resume):
    ...
    for it in range(start, iters+1):
        ctx_x, ctx_y, tgt_x, tgt_y, tokens, true_poly, true_poly_t, _, _ = \
            train_gen.generate_polygon_batch_few_shot_transformation_task()
        ctx_x, ctx_y, tgt_x, tgt_y = [t.to(device) for t in (ctx_x, ctx_y, tgt_x, tgt_y)]
        ...
        if it % plot_after == 0:
            ...
            predicted = Polygon.from_tokenised(pred[:len(tok_orig)].tolist(), tok_orig[0])

            print(f"{datetime.datetime.now():%Y-%m-%d %H:%M:%S}, Iteration: {it}, Test Loss: {t_loss.item():.4f}")
            print("Original:", true_orig)
            print("Gold Transformed:", true_targ)
            print("Predicted Transformed:", predicted)
    ...
    base = f"models/polygon/np/transformation_task/{cfg['model_cls'].__name__}"
    ck = {
