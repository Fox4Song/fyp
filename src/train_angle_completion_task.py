...
def transform_train(model_name, iters, plot_after, device, resume):
    ...
    for it in range(start, iters + 1):
        ctx_x, ctx_y, tgt_x, tgt_y, *_, ctx_mask = (
            train_gen.generate_polygon_batch_few_shot_completion_task()
        )
        ...
        if it % plot_after == 0:
            cx, cy, tx, ty, tok_e, tp, *_, cx_mask = (
                test_gen.generate_polygon_batch_few_shot_completion_task()
            )
            ...
            print("True angle values:      ", true_angles)
            print("Predicted angle values: ", pred_angles)

    # Save final model and checkpoint
    base = f"models/polygon/np/angle_completion_task/{cfg['model_cls'].__name__}"
    ck = {
        "iteration": it,
